...
python -m scripts.baselines.train_ols association-test --trait=$1 --config-path=$2

# python -m scripts.baselines.train_ols association-test --trait=LDL_direct --config-path=run_config_test.yaml
# python -m scripts.baselines.train_ols lm-phenopred --trait=Albumin --config-path=run_config_test.yaml