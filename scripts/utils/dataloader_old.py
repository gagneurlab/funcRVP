...
def load_data(
    ...
    if covariates_path:
        ...
        # covs_dt = covs_dt[covs_dt['unrelated_filter']==True]
        # covs_dt = covs_dt.drop(columns='unrelated_filter')
    else:
    ...
    if trainval_sampling:
        id_trainval = inds[inds.split.isin(['train', 'val'])].sample(frac=trainval_sampling, random_state=0)
        inds = pd.concat([id_trainval, inds[inds.split.isin(['test'])]])
