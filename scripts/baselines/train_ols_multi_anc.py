...
@click.option("--ancestry", type=str, required=False, help="Trait to train model for")
def association_test(
    ...
    if config.get('output_dir_base', None):
        ols_op_dir = os.path.join(config['output_dir_base'], genotype, f"rvat{'_anc' if ancestry else ''}{ancestry if ancestry else ''}{'_onlytest' if only_test else ''}{'_sampling' if trainval_sampling else ''}{trainval_sampling if trainval_sampling else ''}")
    else:
    ...
    (
        ...
        gene_subset = dataloader_params.get('gene_subset', None),
        ancestry = ancestry,
        ancestry_file_path = dataloader_params.get('ancestry_file_path', None),
        unrelated_only = dataloader_params.get('unrelated_only', None),
    )
    ...
    at_df['ancestry'] = ancestry
    at_df["dataset_version"] = dataloader_params.get('unrelated_only', "filteredv3")
